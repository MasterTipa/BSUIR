; Найти кол-во строк в файле, где не встречается указанное слово
...
.data
    ...
    file_path_to_create db "temp.txt", 0
...
.code
    ...
    handle_buffer proc
        ...
        buffer_loop:
            ...
            mov bx, file_descriptor_out
            mov cx, word ptr[letters_count]
    ...
    _start:
        ...
        cmp ax, 0
        ja _exit
        mov di, offset file_descriptor_out
        mov dx, offset file_path_to_create
        call create_file
        mov ax, word ptr[error_code]
        cmp ax, 0
