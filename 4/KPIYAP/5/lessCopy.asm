...
parseCMD PROC                                   ;
	...
	lea si, cmd_text                            ; В si загружаем смещение данных из командой строки
	                                            ;                                                          
	lea di, buffer2                              ;(вызываемый экзешник) В di загружаем смещение буффера для обработки данных      
	call rewriteAsWord                          ;                                                           
                                                ;                                                           
	...
	call rewriteAsWord                          ;
                                                ;
	cmpWordLenWith0 buffer2, argsIsGood          ; Если больше данных нет, т.е. кроме названия выходного и выходного файлов командная строка ничего не содержит
...
loopParseWord:                  ;
	...
	inc di 
	; Увеличиваем di,si т.е. переходим на следующий символ
nextLoopParseWord:
	inc si                      ;
                                ;
...
isStoppedSymbol: 
	...
	inc si 
	; Увеличиваем si для перезода на следующий символ командной строки
    pop dx
	pop bx
